...
void random(int* arr, int total, int a, int b)//{[a,b]的随机数
{
...
{//中值算法来自https://blog.51cto.com/u_15302258/3086800
	...
	int* processing_arr = new int[origin_col * row];//C26451警告 算术溢出
	multiset<int> st;
	...
	for (int i = 0; i < row; i++)
	{
		...
		for (int j = 0; j < origin_col; j++)
		{
			...
			if (i >= n - 1)
			{
				auto mid = st.begin();
				std::advance(mid, n / 2);
				processing_arr[i * origin_col + j] = (*mid + *prev(mid, (static_cast<std::_Iter_diff_t<std::_Tree<std::_Tset_traits<int, std::less<int>, std::allocator<int>, true>>::iterator>>(1) - n % 2))) / 2;//这里有问题
	...
	for (int j = 0; j < origin_col; j++)
	{
		...
		for (int i = 0; i < origin_row; j++)
		{
			...
			if (i >= n - 1)
			{
				auto mid = st1.begin();
				std::advance(mid, n / 2);
				median_arr[i * origin_col + j] = (*mid + *prev(mid, (static_cast<std::_Iter_diff_t<std::_Tree<std::_Tset_traits<int, std::less<int>, std::allocator<int>, true>>::iterator>>(1) - n % 2))) / 2;//C6386 写入到数组时缓冲区溢出
			}
		}
		st1.clear();
	}
...
int main()
{
	...
	print_arr(array, 9, 9);
	int* median_arr = get_median_filter(array, 9, 9, 3);//5行7列 3*3中值滤波
